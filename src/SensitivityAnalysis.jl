...
module SensitivityAnalysis
  ...
  # Which contingencies a factor matrix is evaluated for. One type across the analyses, so
  # it is taken from where it is defined rather than declared again here, and re-exported
  # so a caller can keep writing SensitivityAnalysis.ALL.
  using ..Contingency
  const ContingencyContextType = Contingency.ContingencyContextType
  const ALL = Contingency.ALL
  const NONE = Contingency.NONE
  const SPECIFIC = Contingency.SPECIFIC
  const ONLY_CONTINGENCIES = Contingency.ONLY_CONTINGENCIES
