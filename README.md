...
network refreshed from a file, and a file probed to check whether it is loadable. Only
text formats round-trip through a string; a binary one such as BIIDM needs a byte buffer,
which is not wrapped yet.

```julia
julia> network = Powsybl.Network.load("case.xiidm")

# Export to a string, XIIDM unless another format is given
julia> content = Powsybl.Network.save_to_string(network)
julia> as_json = Powsybl.Network.save_to_string(network, "JIIDM")
...
julia> Powsybl.Network.is_loadable("case.xiidm")
true

# Update an existing network in place from a file (importer-dependent; CGMES supports it)
julia> Powsybl.Network.update_from_file(network, "updated_state.zip")
```
