...
module SecurityAnalysis
  ...
  # The contingency context is one type across the analyses, so it is taken from where it
  # is defined rather than declared again here, and re-exported so a caller can keep
  # writing SecurityAnalysis.ALL.
  using ..Contingency
